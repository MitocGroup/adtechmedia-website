...
fixture `"Contact" form request submit verification`
  .page `https://www-dev.adtechmedia.io/`;
  ...
  test('Check "Contact" form request can be submitted by user with valid data', async t => {
    ...
    // const notificationMessage = await Selector('.noty_bar noty_type_success');
    //
    // await t
    // .debug()
    //   .expect(Selector(notificationMessage).with({
    //     selectorTimeout: 5000,
    //     visibilityCheck: true,
    //   }).visible).ok()
    //   .expect(Selector(notificationMessage).innerText).eql('test');
  });
