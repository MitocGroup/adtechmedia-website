! function() {
	...
	s.parentNode.insertBefore(tk, s)
		...
		var classLength, device, orientationType, viewportClass, eventQueue = {},
			...
			addExperimentClass = function() {
				var i, regex = new RegExp("has-experiment-[\\w]*-[\\d]", "g"),
					classNamesToAdd = [],
					classNames = window.NYTEXPERIMENTS.engine.getClassNamesForExperiments();
				for (i = 0; i < classNames.length; i += 1) classNamesToAdd.push("has-experiment-" + classNames[i]);
				changeClass(classNamesToAdd, regex)
			},
			...
			init = function() {
				return 0 === getClasses().length && setClasses(""), detectDevice(), detectMoz(), detectEmbedPosition(), checkSize(), processFlags(), addAbTestClass(), addExperimentClass(), window.onresize = checkSize, "mobile" === device && (orientationChange(), window.onorientationchange = orientationChange), {
					on: on,
					device: device,
					getWidth: getWidth,
					getHeight: getHeight,
		...
		var bleedType, coverImage, coverImageHeight, coverImageWidth, coverImageCaption, story, storyHeader, headline, storyMeta, storyBody, cover, coverHeight, realImgHeight, realImgWidth, imageAspectRatio, imageOrientation, topCropBias, leftCropBias, bottomCropBias, rightCropBias, coverImageCaptionHeight, windowWidth = window.magnum.getWidth(),
			...
			setStoryBodyTop = function(top) {
				var featureFlags = window.magnum.getFlags(),
					bleedtransitionsFlag = featureFlags.indexOf("bleedtransitions") > -1;
				top += bleedtransitionsFlag ? "full-bleed-cover" !== bleedType || belowCoverToLedeBreakpoint ? 20 : coverImageCaptionHeight > 0 ? coverImageCaptionHeight + 65 : 80 : "full-bleed-cover" === bleedType && windowWidth > coverToLedeBreakpoint ? coverImageCaptionHeight > 0 ? coverImageCaptionHeight + 65 : 80 : 20, storyBody.style.marginTop = top + "px"
			},
			...
			setCoverHeight = function(newCoverHeight, newCaptionPosition) {
				var featureFlags = window.magnum.getFlags(),
					bleedtransitionsFlag = featureFlags.indexOf("bleedtransitions") > -1;
				cover.style.height = newCoverHeight + "px", setCaptionPosition(bleedtransitionsFlag ? newCaptionPosition : coverToLedeBreakpoint > windowWidth ? coverImageHeight : newCoverHeight)
			},
			...
			handleCropBias = function(offset, direction) {
				...
				return "horizontal" === direction && (pixelsCropped = fullImageWidth - coverWidth, horizontalBias = (rightCropBias - leftCropBias) / 100, -.5 >= horizontalBias && (horizontalBias = -.5), offset += pixelsCropped * horizontalBias, offset >= 0 && (offset = 0)), "vertical" === direction && (pixelsCropped = fullImageHeight - coverHeight, verticalBias = (bottomCropBias - topCropBias) / 100, -.5 >= verticalBias && (verticalBias = -.5), offset += pixelsCropped * verticalBias, offset >= 0 && (offset = 0)), offset
			},
			...
			fitImage = function() {
				...
				getAllDimensions(), imageScaleRatio = realImgHeight / realImgWidth, coverScaleRatio = coverHeight / coverWidth, imageScaleRatio > coverScaleRatio ? cropImageHorizontally() : coverScaleRatio > imageScaleRatio && ("full-bleed-lede" === bleedType ? scaleImageHorizontally() : cropImageVertically())
			},
			...
			makeFullBleedLede = function() {
				var newCoverHeight, percentageOfVerticalImageVisable, imageHeight, featureFlags = window.magnum.getFlags(),
					bleedtransitionsFlag = featureFlags.indexOf("bleedtransitions") > -1;
				getAllDimensions(), newCoverHeight = windowHeight - headline.clientHeight - coverImageCaptionHeight, percentageOfVerticalImageVisable = newCoverHeight / coverImageHeight * 100, imageHeight = coverImageHeight, coverWidth = windowWidth, bleedtransitionsFlag ? belowCoverToLedeBreakpoint ? (setCoverHeight(imageHeight + coverImageCaptionHeight, coverImageHeight), setStoryBodyTop(storyHeader.clientHeight)) : (fullBleedImageVerticalBreakpoint > percentageOfVerticalImageVisable && "landscape" === imageOrientation && (newCoverHeight = .01 * fullBleedImageVerticalBreakpoint * coverImageHeight), setCoverHeight(newCoverHeight, newCoverHeight), setStoryBodyTop(storyHeader.clientHeight), fitImage()) : windowWidth > coverToLedeBreakpoint ? (fullBleedImageVerticalBreakpoint > percentageOfVerticalImageVisable && "landscape" === imageOrientation && (newCoverHeight = .01 * fullBleedImageVerticalBreakpoint * coverImageHeight), setCoverHeight(newCoverHeight), setStoryBodyTop(storyHeader.clientHeight), fitImage()) : (setCoverHeight(imageHeight + coverImageCaptionHeight), setStoryBodyTop(storyHeader.clientHeight))
			},
			setCoverImage = function(imageId, bleedStyle, imgHeight, imgWidth, cropTop, cropRight, cropBottom, cropLeft) {
				var featureFlags = window.magnum.getFlags(),
					bleedtransitionsFlag = featureFlags.indexOf("bleedtransitions") > -1;
				cover = document.getElementById("media-" + imageId), coverImage = document.getElementById("media-image-" + imageId), coverImageCaption = document.getElementById("media-caption-" + imageId), story = document.getElementById("story"), storyHeader = document.getElementById("story-header"), headline = document.getElementById("headline"), storyMeta = document.getElementById("story-meta"), storyBody = document.getElementsByClassName("story-body-supplemental")[0], realImgHeight = imgHeight, realImgWidth = imgWidth, bleedType = bleedStyle, imageAspectRatio = Math.min(realImgWidth / realImgHeight), topCropBias = cropTop, leftCropBias = cropLeft, bottomCropBias = cropBottom, rightCropBias = cropRight, imageOrientation = realImgHeight > realImgWidth ? "portrait" : "landscape", coverToLedeBreakpoint > windowWidth ? (story.className = story.className.replace(/(?:^|\s)has-full-bleed-cover(?!\S)/g, ""), story.className += " has-full-bleed-image-lede", bleedtransitionsFlag && setBelowCoverToLedeBreakpoint(!0), makeFullBleedLede()) : "full-bleed" === bleedType ? makeFullBleedLede() : "full-bleed-cover" === bleedType && makeFullBleedCover()
			};
		return {
		...
		return e.input || J(), e.addTest = function(a, b) {
			if ("object" == typeof a)
				...
				b = "function" == typeof b ? b() : b,
					"undefined" != typeof f && f && (g.className += " has-" + (b ? "" : "no-") + a), e[a] = b
			}
			return e
...
function(window) {
	var BlockAdBlock = function(options) {
		...
		var self = this,
			eventCallback = function() {
				setTimeout(function() {
					self._options.checkOnLoad === !0 && (self._options.debug === !0 && self._log("onload->eventCallback", "A check loading is launched"), null === self._var.bait && self._creatBait(), setTimeout(function() {
						self.check()
					}, 1))
				}, 1)
			};
		void 0 !== window.addEventListener ? window.addEventListener("load", eventCallback, !1) : window.attachEvent("onload", eventCallback)
	};
	BlockAdBlock.prototype._options = null, BlockAdBlock.prototype._var = null, BlockAdBlock.prototype._bait = null, BlockAdBlock.prototype._log = function(method, message) {
		...
		for (var option in options) this._options[option] = options[option], this._options.debug === !0 && this._log("setOption", 'The option "' + option + '" he was assigned to "' + options[option] + '"');
		return this
	}, BlockAdBlock.prototype._creatBait = function() {
		var bait = document.createElement("div");
		bait.setAttribute("class", this._options.baitClass), bait.setAttribute("style", this._options.baitStyle), this._var.bait = window.document.body.appendChild(bait), this._var.bait.offsetParent, this._var.bait.offsetHeight, this._var.bait.offsetLeft,
			this._var.bait.offsetTop, this._var.bait.offsetWidth, this._var.bait.clientHeight, this._var.bait.clientWidth, this._options.debug === !0 && this._log("_creatBait", "Bait has been created")
	}, BlockAdBlock.prototype._destroyBait = function() {
		window.document.body.removeChild(this._var.bait), this._var.bait = null, this._options.debug === !0 && this._log("_destroyBait", "Bait has been removed")
	}, BlockAdBlock.prototype.check = function(loop) {
		if (void 0 === loop && (loop = !0), this._options.debug === !0 && this._log("check", "An audit was requested " + (loop === !0 ? "with a" : "without") + " loop"), this._var.checking === !0) return this._options.debug === !0 && this._log("check", "A check was canceled because there is already an ongoing"), !1;
		this._var.checking = !0, null === this._var.bait && this._creatBait();
		var self = this;
		return this._var.loopNumber = 0, loop === !0 && (this._var.loop = setInterval(function() {
		...
		var detected = !1;
		if (null === this._var.bait && this._creatBait(), null === window.document.body.getAttribute("abp") && null !== this._var.bait.offsetParent && 0 != this._var.bait.offsetHeight && 0 != this._var.bait.offsetLeft && 0 != this._var.bait.offsetTop && 0 != this._var.bait.offsetWidth && 0 != this._var.bait.clientHeight && 0 != this._var.bait.clientWidth || (detected = !0), void 0 !== window.getComputedStyle) {
			var baitTemp = window.getComputedStyle(this._var.bait, null);
			"none" != baitTemp.getPropertyValue("display") && "hidden" != baitTemp.getPropertyValue("visibility") || (detected = !0)
		}
		this._options.debug === !0 && this._log("_checkBait", "A check (" + (this._var.loopNumber + 1) + "/" + this._options.loopMaxNumber + " ~" + (1 + this._var.loopNumber * this._options.loopCheckTime) + "ms) was conducted and detection is " + (detected === !0 ? "positive" : "negative")), loop === !0 && (this._var.loopNumber++, this._var.loopNumber >= this._options.loopMaxNumber && this._stopLoop()), detected === !0 ? (this._stopLoop(), this._destroyBait(), this.emitEvent(!0), loop === !0 && (this._var.checking = !1)) : null !== this._var.loop && loop !== !1 || (this._destroyBait(), this.emitEvent(!1), loop === !0 && (this._var.checking = !1))
		...
		this._options.debug === !0 && this._log("emitEvent", "An event with a " + (detected === !0 ? "positive" : "negative") + " detection was called");
		var fns = this._var.event[detected === !0 ? "detected" : "notDetected"];
		for (var i in fns) this._options.debug === !0 && this._log("emitEvent", "Call function " + (parseInt(i) + 1) + "/" + fns.length), fns.hasOwnProperty(i) && fns[i]();
		...
		return this._var.event[detected === !0 ? "detected" : "notDetected"].push(fn), this._options.debug === !0 && this._log("on", 'A type of event "' + (detected === !0 ? "detected" : "notDetected") + '" was added'), this
	}, BlockAdBlock.prototype.onDetected = function(fn) {
		return this.on(!0, fn)
	}, BlockAdBlock.prototype.onNotDetected = function(fn) {
		return this.on(!1, fn)
	}, window.BlockAdBlock = BlockAdBlock, void 0 === window.blockAdBlock && (window.blockAdBlock = new BlockAdBlock({
		checkOnLoad: !1,
		resetOnEnd: !0
	}))
}(window);
